...
<script>
    ...
    layui.config({
            ...
            onSuccess: function ($elem) { //$elem当前窗口的jq对象
                $elem.children('tr').each(function () {
                    ...
                    $(this).children('td').each(function () {
                        ...
                        if (field == "Value"){
                            $(this).html(HTMLEncode(field_value))
                        }
                    });
                    $(this).children('td:last-child').children('input').each(function () {
                        ...
                        $that.on('click', function () {
                            switch (action) {
                                case 'edit':
                                    $.get('/admin/site/edit_site/'+id, null, function(form) {
                                        addBoxIndex = layer.open({
                                            ...
                                            success: function(layero, index) {
                                                ...
                                                form.on('submit(edit)', function(data) {
                                                    //调用父窗口的layer对象
                                                    baajax.post('/admin/site/post_edit_site', data.field, function(ret){
                                ...
                                case 'del': //删除
                                    ...
                                    layerTips.confirm('确定要删除[ <span style="color:red;">' + name + '</span> ] ？', { icon: 3, title: '系统提示' }, function (index) {
                                        layerTips.close(index);
                                        baajax.post("/admin/site/delete_select", {ids:id}, function(res){
                                            layerTips.close()
                                            layer.alert(res.msg);
                                            setTimeout(function () {
                                                location.reload();
        ...
        $("#Add").on("click", function () {
            if(addBoxIndex !== -1)
                return;
            $.get('/admin/site/add_site', null, function(form) {
                addBoxIndex = layer.open({
                    type: 1,
                    title: '添加配置',
                    content: form,
                    ...
                    success: function(layero, index) {
                        ...
                        form.on('submit(edit)', function(data) {
                            //调用父窗口的layer对象
                            baajax.post('/admin/site/post_add_site', data.field, function(ret){
                                layerTips.msg(ret.msg);
                                layerTips.close(index);
                                location.reload(); //刷新
                            });
