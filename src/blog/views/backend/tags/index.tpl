...
<script>
    layui.config({
                ...
                $elem.children('tr').each(function () {
                    $(this).children('td:last-child').children('input').each(function () {
                        ...
                        $that.on('click', function () {
                            switch (action) {
                                case 'edit':
                                    $.get('/admin/tag/edit_tag/'+id, null, function(form) {
                                ...
                                case 'del': //删除
                                    ...
                                    layerTips.confirm('确定要删除[ <span style="color:red;">' + name + '</span> ] ？', { icon: 3, title: '系统提示' }, function (index) {
                                        layerTips.close(index);
                                        baajax.post("/admin/tag/delete_select", {ids:id}, function(res){
                                            layerTips.close()
                                            layer.alert(res.msg);
                                            setTimeout(function () {
                                                location.reload();
        ...
        $("#Add").on("click", function () {
            if(addBoxIndex !== -1)
                return;
            $.get('/admin/tag/add_tag', null, function(form) {
                addBoxIndex = layer.open({
                    type: 1,
                    title: '添加分类',
                    content: form,
                    ...
                    success: function(layero, index) {
                        ...
                        form.on('submit(edit)', function(data) {
                            //调用父窗口的layer对象
                            baajax.post('/admin/tag/post_add_tag', data.field, function(ret){
                                layerTips.msg(ret.msg);
                                layerTips.close(index);
                                location.reload(); //刷新
                            });
