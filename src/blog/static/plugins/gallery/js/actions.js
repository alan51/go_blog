var page_actions = function(){
    ...
    $(".gallery-selected-remove").on("click", function () {
        var ids = ""
        var csrf = $("input[name=_xsrf]").val()
        data = {"ids":ids, "_xsrf":csrf}
        ...
        if(!data.ids) {
            layui.layer.alert("请选择要删除的图片")
            return false
        }
        $.ajax({
            url:"/admin/file/delete_select",
            type:"POST",
            data:data,
