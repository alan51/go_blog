tbfine(function(){return{init:function(){var signHtml='\
    ...
    $('.signin-loader').on('click',function(){jsui.bd.addClass('sign-show')
        setTimeout(function(){$('#sign-in').show().find('input:first').focus()},300);$('#sign-up').hide()})
    ...
    $('.signsubmit-loader').on('click',function(){if(jsui.is_signin)return
        ...
        if(isreg){if(!is_mail(inputs.email)){logtips('邮箱格式错误')
            return}
            if(!/^[a-z\d_]{3,20}$/.test(inputs.name)){logtips('昵称是以字母数字下划线组合的3-20位字符')
                return}}else{if(inputs.password.length<6){logtips('密码太短，至少6位')
            return}}
        $.ajax({type:"POST",url:jsui.uri+'/action/log.php',data:inputs,dataType:'json',success:function(data){if(data.msg){logtips(data.msg)}
            if(data.error){return}
            if(!isreg){location.reload()}else{if(data.goto)location.href=data.goto}}});})
    var _loginTipstimer
