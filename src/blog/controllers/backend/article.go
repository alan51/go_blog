...
func (c *ArticleController) URLMapping() {
	...
	c.Mapping("EditArticle", c.EditArticle)
}
...
func (c *ArticleController) DeleteSelect() {
	var ids []string
	...
	err := models.DeleteArticlesByIds(ids)
	if err != nil {
...
// @Description 新增页面显示
// @Success 200 {object} 返回是否 cgong 成功
// @router /add_article [get]
func (c *ArticleController) AddArticle() {
	c.Data["xsrf_token"] = template.HTML(c.XSRFToken())
	tag, _ := models.GetTags()
	c.Data["tags"] = tag
	category, _ := models.GetCategories()
	c.Data["category"] = category
	c.TplName = beego.AppConfig.String("admin_view_path") + "articles/create.tpl"
}
...
func (c *ArticleController) PostAddArticle() {
	...
	m.Category = category
	session_user := c.GetSession("userinfo").(models.Users)
	User := new(models.Users)
	User.Id = session_user.Id
	m.User = User
	_, err := models.AddArticles(&m)
	if err != nil {
		c.Rsp(403, err.Error())
		return
	} else {
...
func (c *ArticleController) EditArticle() {
	...
	c.Data["article_info"] = v
	tag, _ := models.GetTags()
	c.Data["tags"] = tag
	c.Data["xsrf_token"] = template.HTML(c.XSRFToken())
	category, _ := models.GetCategories()
	c.Data["category"] = category
	c.TplName = beego.AppConfig.String("admin_view_path") + "articles/edit.tpl"
}
...
// @router /post_edit_article [post]
func (c *ArticleController) PostEditArticle() {
	m := models.Articles{}
	if err := c.ParseForm(&m); err != nil {
		c.Rsp(403, err.Error())
		return
	...
	User.Id = session_user.Id
	m.User = User
	err := models.UpdateArticlesById(&m)
	if err != nil {
		c.Rsp(201, err.Error())
	} else {
		c.Rsp(200, "编辑成功")
...
func (c *ArticleController) Index() {
	if c.IsAjax() == false {
		...
		c.TplName = beego.AppConfig.String("admin_view_path") + "articles/index.tpl"
	} else {
		var fields []string
		var sortby []string
		var order []string
		...
		l, count, err := models.GetAllArticles(query, fields, sortby, order, offset, limit)
		if err != nil {
