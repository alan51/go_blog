...
func (c *LinkController) DeleteSelect() {
	var ids []string
	...
	err := models.DeleteLinksByIds(ids)
	if err != nil {
...
func (c *LinkController) AddLink() {
	...
	c.TplName = beego.AppConfig.String("admin_view_path") + "links/create.tpl"
}
...
// @router /post_add_link [post]
func (c *LinkController) PostAddLink() {
	m := models.Links{}
	if err := c.ParseForm(&m); err != nil {
		c.Rsp(403, err.Error())
		return
	}
	_, err := models.AddLinks(&m)
	if err != nil {
		c.Rsp(403, err.Error())
		return
	} else {
...
// @router /edit_link/:id [get]
func (c *LinkController) EditLink() {
	...
	c.Data["link_info"] = v
	c.Data["xsrf_token"] = template.HTML(c.XSRFToken())
	c.TplName = beego.AppConfig.String("admin_view_path") + "links/edit.tpl"
}
...
// @router /post_edit_link [post]
func (c *LinkController) PostEditLink() {
	m := models.Links{}
	if err := c.ParseForm(&m); err != nil {
		c.Rsp(403, err.Error())
		return
	}
	err := models.UpdateLinksById(&m)
	if err != nil {
		c.Rsp(201, err.Error())
	} else {
		c.Rsp(200, "编辑成功")
...
func (c *LinkController) Index() {
	if c.IsAjax() == false {
		...
		c.TplName = beego.AppConfig.String("admin_view_path") + "links/index.tpl"
	} else {
		var fields []string
		var sortby []string
		var order []string
		...
		l, count, err := models.GetAllLinks(query, fields, sortby, order, offset, limit)
		if err != nil {
