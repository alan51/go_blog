...
func (c *TagController) DeleteSelect() {
	var ids []string
	...
	err := models.DeleteTagsByIds(ids)
	if err != nil {
...
func (c *TagController) AddTag() {
	...
	c.TplName = beego.AppConfig.String("admin_view_path") + "tags/create.tpl"
}
...
// @router /post_add_tag [post]
func (c *TagController) PostAddTag() {
	m := models.Tags{}
	if err := c.ParseForm(&m); err != nil {
		c.Rsp(403, err.Error())
		return
	}
	_, err := models.AddTags(&m)
	if err != nil {
		c.Rsp(403, err.Error())
		return
	} else {
...
// @router /edit_tag/:id [get]
func (c *TagController) EditTag() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.ParseInt(idStr, 0, 64)
	v, err := models.GetTagsById(id)
	if err != nil {
		return
	}
	c.Data["tag_info"] = v
	c.Data["xsrf_token"] = template.HTML(c.XSRFToken())
	c.TplName = beego.AppConfig.String("admin_view_path") + "tags/edit.tpl"
}
...
// @router /post_edit_tag [post]
func (c *TagController) PostEditTag() {
	m := models.Tags{}
	if err := c.ParseForm(&m); err != nil {
		c.Rsp(403, err.Error())
		return
	}
	err := models.UpdateTagsById(&m)
	if err != nil {
		c.Rsp(201, err.Error())
	} else {
		c.Rsp(200, "编辑成功")
...
func (c *TagController) Index() {
	if c.IsAjax() == false {
		...
		c.TplName = beego.AppConfig.String("admin_view_path") + "tags/index.tpl"
	} else {
		var fields []string
		var sortby []string
		var order []string
