...
func (c *CategoryController) DeleteSelect() {
	var ids []string
	...
	err := models.DeleteCategoriesByIds(ids)
	if err != nil {
...
func (c *CategoryController) EditCategory() {
	...
	c.Data["category_info"] = v
	c.Data["xsrf_token"] = template.HTML(c.XSRFToken())
	category, _ := models.GetCategories()
	c.Data["category"] = category
	c.TplName = beego.AppConfig.String("admin_view_path") + "categories/edit.tpl"
}
...
// @router /post_edit_category [post]
func (c *CategoryController) PostEditCategory() {
	m := models.Categories{}
	if err := c.ParseForm(&m); err != nil {
		c.Rsp(403, err.Error())
		return
	...
	ParentId, _ := c.GetInt64("ParentId")
	m.ParentId = ParentId
	err := models.UpdateCategoriesById(&m)
	if err != nil {
		c.Rsp(201, err.Error())
	} else {
		c.Rsp(200, "编辑成功")
...
func (c *CategoryController) Index() {
	if c.IsAjax() == false {
		...
		c.TplName = beego.AppConfig.String("admin_view_path") + "categories/index.tpl"
	} else {
		var fields []string
		var sortby []string
		var order []string
		...
		l, count, err := models.GetAllCategories(query, fields, sortby, order, offset, limit)
		if err != nil {
