...
func (c *SiteController) URLMapping() {
	...
	c.Mapping("AddSite", c.AddSite)
	c.Mapping("EditSite", c.EditSite)
}
...
func (c *SiteController) DeleteSelect() {
	var ids []string
	...
	err := models.DeleteSitesByIds(ids)
	if err != nil {
...
func (c *SiteController) AddSite() {
	...
	c.TplName = beego.AppConfig.String("admin_view_path") + "sites/create.tpl"
}
...
// @router /post_add_site [post]
func (c *SiteController) PostAddSite() {
	m := models.Sites{}
	if err := c.ParseForm(&m); err != nil {
		c.Rsp(403, err.Error())
		return
	}
	_, err := models.AddSites(&m)
	if err != nil {
		c.Rsp(403, err.Error())
		return
	} else {
...
// @Success 200 {object} models.Sites
// @router /edit_site/:id [get]
func (c *SiteController) EditSite() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.ParseInt(idStr, 0, 64)
	v, err := models.GetSitesById(id)
	if err != nil {
		return
	}
	c.Data["site_info"] = v
	c.Data["xsrf_token"] = template.HTML(c.XSRFToken())
	c.TplName = beego.AppConfig.String("admin_view_path") + "sites/edit.tpl"
}
...
// @router /post_edit_site [post]
func (c *SiteController) PostEditSite() {
	m := models.Sites{}
	if err := c.ParseForm(&m); err != nil {
		c.Rsp(403, err.Error())
		return
	}
	err := models.UpdateSitesById(&m)
	if err != nil {
		c.Rsp(201, err.Error())
	} else {
		c.Rsp(200, "编辑成功")
...
func (c *SiteController) Index() {
	if c.IsAjax() == false {
		...
		c.TplName = beego.AppConfig.String("admin_view_path") + "sites/index.tpl"
	} else {
		var fields []string
		var sortby []string
		var order []string
		...
		l, count, err := models.GetAllSites(query, fields, sortby, order, offset, limit)
		if err != nil {
